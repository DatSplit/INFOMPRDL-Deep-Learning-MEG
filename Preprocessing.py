...
ROOT_DIRECTORIES = {"Intra": "data/Intra", "Cross": "data/Cross"}
...
def extract_dataset_name(file_path):
    ...
    match = re.search(
        r"(rest|task_working_memory|task_motor|task_story_math)_\d+", filename
    )
...
def extract_task_name(file_path):
    match = re.search(
        r"(rest|task_working_memory|task_motor|task_story_math)_\d+", file_path
    )
    print(match.group(1))
    if match:
...
def preprocess_MEG_data(root_dir, task):
    ...
    output_dir_scale = "preprocessed_scale_only"
        ...
        for filename in filenames:
            if filename.endswith(".h5"):
                ...
                with h5py.File(file_path, "r") as f:
                    dataset_name = extract_dataset_name(rf"{file_path}")
                    ...
                    X_standardized = (data.T - mean[dirpath]) / std_dev[dirpath]
                    np.savez(
                        npy_output_path_scale,
                        data=X_standardized,
                        task=current_task,
                        label=label,
                    )
                    ...
                    info = mne.create_info(
                        ch_names=channel_names, ch_types=channel_types, sfreq=sfreq
                    )
                    ...
                    raw_notch_lowpass = standardized.copy().filter(
                        l_freq=None, h_freq=333
                    )
                    ...
                    np.savez(npy_output_path, data=data, task=current_task, label=label)
